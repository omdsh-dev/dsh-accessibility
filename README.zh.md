...
npm companion 不会修改 DSH 自有组件。在改动进入官方 DSH 发行版之前，请使用组织固定的 [DSH 无障碍构建](https://github.com/omdsh-dev/deepseek-harness/releases/tag/dsh-v0.1.2-rc.1-a11y.5)，体验完整候选行为：
...
`0.1.0-beta.8` 会通过 DSH 官方 `conversation.view` slot 注册实验性的“无障碍视图”。它仍是 beta 功能，不构成稳定支持声明。
