...
The archive retains the first reviewed `33eb2d9e1e` record, the separately regenerated `5803bfcfdd` record for the exact primary-campaign candidate, the independent `21859d968c` report for the initial DSH `0.1.2-rc.1` accessibility port, and the current `1e7f105934` report after the queue snapshot portability fix. Later commits do not inherit any result automatically.
