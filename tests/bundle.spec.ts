...
describe('browser bundle registration', () => {
  ...
  it('pins the exact DSH 0.1.5-rc.2 client graph and excludes the retired runtime package', () => {
    const manifestPath = fileURLToPath(new URL('../package.json', import.meta.url))
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf8')) as {
      version?: string
    ...
    expect(manifest.version).toBe('0.1.1-beta.1')
    expect(manifest.peerDependencies?.['@deepseek-ai/dsh-client-runtime']).toBeUndefined()
    expect(manifest.peerDependencies?.['@deepseek-ai/dsh-client-ui-chat']).toBe('0.1.5-rc.2')
    expect(manifest.peerDependencies?.['@deepseek-ai/dsh-client-ui-session']).toBe('0.1.5-rc.2')
    expect(Object.entries(manifest.peerDependencies ?? {})
      .filter(([name]) => name.startsWith('@deepseek-ai/dsh-client-'))
      .every(([, version]) => version === '0.1.5-rc.2')).toBe(true)
